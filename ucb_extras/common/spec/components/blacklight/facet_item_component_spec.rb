...
RSpec.describe Blacklight::FacetItemComponent, type: :component do
  ...
  it 'links to the facet and shows the number of hits, overriding Blacklight to add sr_alert and focus_target to the URL' do
    ...
    expect(rendered).to have_link 'x', href: '/catalog?f=x&sr_alert=Added+%3A+%22x%22+to+search+constraints&focus_target=%23remove-facet-x-instancedouble-blacklight-facetitempresenter-anonymous&focus_target=%23facet-x-toggle-btn&focus_target=%23facet-panel-collapse-toggle-btn' do |link|
      link['rel'] == 'nofollow'
    end
  ...
  context 'with a selected facet' do
    ...
    it 'links to the facet and shows the number of hits, overriding Blacklight to add sr_alert and focus_target to the URL and customize the Remove button' do
      ...
      expect(rendered).to have_link 'Remove constraint : x', href: '/catalog?sr_alert=Removed+%3A+%22x%22+from+search+constraints&focus_target=%23add-facet-x-instancedouble-blacklight-facetitempresenter-anonymous&focus_target=%23facet-x-toggle-btn&focus_target=%23facet-panel-collapse-toggle-btn' do |link|
        link['rel'] == 'nofollow'
      end
