  ...
  def generate_garden_bed_preview(garden_bed)
    ...
    docs.collect do |doc|
      content_tag(:div, class: 'show-preview-item') do
        content_tag(:a, href: "/catalog/#{doc[:objcsid_s]}", label: "link to record for botanical garden item #{doc[:canonicalNameComplete_s]}") do
        unless doc[:canonicalNameComplete_s].nil?
          canonical_name = doc[:canonicalNameComplete_s]
        else
          canonical_name = "[No name available]"
        end
        ...
        unless doc[:locality_s].nil?
          ...
          country = "[No collection place available]"
        end
        unless doc[:blob_ss].nil?
          ...
          image_tag = content_tag(:span,'Image not available', class: 'gallery-caption-date', label: "image not available for this item")
        end
        image_tag + 
        content_tag(:span, canonical_name, class: "gallery-caption-title", label: "plant scientific name") +
        content_tag(:span, "("+country+")", class: "gallery-caption-date", label: "plant collecting place") +
        content_tag(:span, "Common name: "+commonname, class: "gallery-caption-artist", label: "plant common name")
